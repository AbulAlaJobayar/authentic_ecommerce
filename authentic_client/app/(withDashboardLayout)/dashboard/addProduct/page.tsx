...
const inventorySchemaValidation = z.object({
  alertQuantity: z.coerce.number().min(1, "Alert Quantity required"),
});
...
const productBatchSchemaValidation = z.object({
  ...
  buyingPrice: z.coerce.number().min(1, "Buying price required"),

  costPrice: z.coerce.number().min(1, "Cost price required"),

  sellingPrice: z.coerce.number().min(1, "Selling price required"),

  shelfCode: z.string().regex(/^S-\d{2}[A-Z]$/, "Format: S-01A"),

  rackCode: z.string().regex(/^R-\d{2}$/, "Format: R-01"),
...
const AddProduct = () => {
  ...
  const { data: categoriesData } = useAllCategoryQuery({ page: 1, limit: 100 });
  console.log(categoriesData, "category data");
        ...
        >
          <FieldGroup className="space-y-10">
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
                <Field className="space-y-2">
                  <FieldLabel htmlFor="shelfCode">Shelf Code</FieldLabel>
                ...
                <Field className="space-y-2">
                  <FieldLabel htmlFor="rackCode">Rack Code</FieldLabel>
                ...
                <Field className="space-y-2">
                  ...
                  <ATSSelect
                    ...
                    options={
                      suppliersData?.data?.data.map((cat: any) => ({
                        label: cat.name,
                        value: cat.id,
                      })) || []
                    }
                ...
                <Field className="space-y-2">
                  ...
                  <ATSSelect
                    ...
                    options={
                      warehousesData?.data?.data.map((cat: any) => ({
                        label: cat.name,
                        value: cat.id,
                      })) || []
                    }
            ...
            {isLoading ? (
              <div>
                <Button disabled className="animate-bounce w-full">
